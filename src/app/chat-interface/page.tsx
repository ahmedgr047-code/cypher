...
export default function ChatPage() {
  ...
  const handleSendMessage = async (text: string, attachment?: AttachedFile) => {
    ...
    try {
      ...
      console.error('[ChatPage] AI Error:', errorMsg);
  ...
  if (!bootstrapped || !activeConvId) {
    return (
      <div className="flex h-screen items-center justify-center bg-black text-red-400 text-sm">
        جاري التحميل…
      </div>
    );
  }
    ...
    <NewChatInterface
      ...
      onWorkbenchToggle={() => {
        const codeBlocks = messages
          .filter(msg => msg.role === 'bot' && msg.content)
          .flatMap(msg => {
            const codeRegex = /```(\w+)?\n([\s\S]*?)\n```/g;
            const matches = [];
            let match;
        ...
        if (codeBlocks.length > 0) {
          // openWorkbench(codeBlocks);
          console.log('Open workbench with code blocks:', codeBlocks);
        }
      }}
      workbenchOpen={false}
      persona={{ name: 'فتيحة', personality: 'مساعدة ودودة للطلاب' }}
    />
  );
}