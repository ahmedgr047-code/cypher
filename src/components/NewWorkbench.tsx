...
import { Code2, Play, Copy, Download, Expand, X, Terminal } from "lucide-react";
...
export default function NewWorkbench({ 
  isOpen, 
  onClose, 
  codeBlocks, 
  onRunCode,
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0, y: 20 }}
        className={cn(
          "bg-black border-t border-red-500/30 z-50",
          isSplitScreen 
            ? "fixed inset-0 h-full" 
            : "fixed bottom-0 left-0 right-0 h-96 lg:h-1/2"
        )}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-red-500/20 bg-black/80 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => {
                // Toggle split screen
                console.log('Toggle split screen');
              }}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4 border-t border-red-500/20 bg-black/80 backdrop-blur-sm">
            <div className="flex items-center gap-2">
              <Button
                size="sm"
                onClick={() => handleRun(codeBlocks[activeTab].content, codeBlocks[activeTab].language)}
                disabled={isRunning}
                className="bg-red-600 hover:bg-red-700 text-white border border-red-500/30 disabled:opacity-50"
              >
                {isRunning ? (
                  <motion.div
                    animate={{ rotate: 360 }}
